...
class Tree_t {
    ...
    public:
        ...
        void free_subtree(Node_t<keyT, valT>* r) {
            if(r->left) free_subtree(r->left);
            if(r->right) free_subtree(r->right);
            delete r;
            return;
        }
        ...
        int size(Node_t<keyT, valT>* n) {
            if (!n) { return 0; }
                ...
                int val = 1;
                if (n->left) { val += size(n->left); }
                if (n->right) { val += size(n->right); }
        ...
        Return_t<valT> search(keyT key) {
            if(!root) return {valT(),false};
            Node_t<keyT, valT>* n = search_subtree(root, key);
            if (n && n->key == key) return {n->value, true};
            else return {valT(), false};
        }
        ...
        bool check_order(void) {
            if(root) return check_order_subtree(root);
            else return true;
        }
        ...
        void dump_subtree(Node_t<keyT, valT>* root, string prefix=string("")) {
            ...
            if (root->left) dump_subtree(root->left, prefix + string("l-"));
            if (root->right) dump_subtree(root->right, prefix + string("r-"));
        }
        ...
        void rotate(Node_t<keyT, valT>* n, bool direction) {
            ...
            if(direction){
                if(root == n){
                    Node_t<keyT, valT>* temp = root->left;
                    ...
                    Node_t<keyT, valT>* temp_parent = n->parent;
                    bool right = false;
                    if(n->parent->right){
                ...
                if(root == n){
                    Node_t<keyT, valT>* temp = root->right;
...
//  g++ -o rotate Tree_rotate_test.cpp -Wall -std=c++11 -O3
