...
int main(int argc, char *argv[]){
    ...
    int sock1, sock2;
    // declare ip/port
    struct sockaddr_in sock1_client, sock2_client;
    ...
    // 4. accept()
    socklen_t client_size_1 = sizeof(sock1_client);
    sock1 = accept(proxy_sock, (struct sockaddr*)&sock1_client, &client_size_1);
    if(sock1 < 0){
        printf("CAN'T ACCEPT\n");
        abort();
    }
    // 4. accept()
    socklen_t client_size_2 = sizeof(sock2_client);
    sock2 = accept(proxy_sock, (struct sockaddr*)&sock2_client, &client_size_2);
    if(sock2 < 0){
        printf("CAN'T ACCEPT\n");
        abort();
    }
    ...
    if(select(MAX(sock1, sock2) + 1, &read_set, NULL, NULL, &timeout) < 0){
        printf("SELCET ERROR\n");
        abort();
    }
    if(FD_ISSET(sock1, &read_set)){
        ...
        int client_receive = recv(sock1, client_buffer, expected_data_len, 0);
        if(client_receive < 0){
        ...
        if(valid == 0){
            ...
            strcat(send_msg, "\nConnection: close\n\n");
            FILE *file;
            char *filename = origin_port + ".html";
            ...
            if(file){
                ...
                fputs(origin_buffer, file);
                fclose(file);
                // 6. write
                // printf("%s\n", origin_buffer);
                write(sock1, origin_buffer, expected_data_len);
                close(send_sock);   
            }
        }
        else if(valid == 1)
        write(sock1, "Bad Request(400)\n", sizeof("Bad Request(400)\n"));
        else
        write(sock1, "Not Implemented(501)\n", sizeof("Not Implemented(501)\n"));
    }
    if(FD_ISSET(sock2, &read_set)){
        // sock2 has data
        char client_buffer[4096];
        int expected_data_len = sizeof(client_buffer);
        int client_receive = recv(sock2, client_buffer, expected_data_len, 0);
        if(client_receive < 0){
            printf("RECEIVE ERROR\n");
            return 1;
        }
        ...
        if(valid == 0){
            ...
            // 6. write
            // printf("%s\n", origin_buffer);
            write(sock2, origin_buffer, expected_data_len);
            close(send_sock);
        }
        else if(valid == 1)
        write(sock2, "Bad Request(400)\n", sizeof("Bad Request(400)\n"));
        else
        write(sock2, "Not Implemented(501)\n", sizeof("Not Implemented(501)\n"));
    }
    // 8. close
    close(sock1);
    close(sock2);
...
// Save에 대한 부분 : 
// I/O multiplexing에 대한 부분 - ppt에 있음(이해 해야할 듯)